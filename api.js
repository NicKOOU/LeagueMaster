...
async function showAllRunesInConsoleToChoose (championId, assignedPosition, rune) {
  ...
  await Promise.race([runeIndexPromise, timeout])
    .then(async (answer) => {
      if (typeof answer === 'string') {
        ...
        const runes = await parseDataBaseRune(databaseRune, championId, runeIndex, rune)
        const newrune = new Rune(runes.primaryStyleId, runes.subStyleId, runes.selectedPerkIds, runes.name)
...
async function parseDataBaseRune (databaseRune, name, index, runes) {
  ...
  runes.selectedPerkIds = [databaseRune.data[index].primary1, databaseRune.data[index].primary2, databaseRune.data[index].primary3, databaseRune.data[index].primary4, databaseRune.data[index].sub1, databaseRune.data[index].sub2, shards[0], shards[1], shards[2]]
  return runes
}
...
async function champandrune (data, lcu) {
  ...
  resp.myTeam.forEach(champ => {
    if (champ.championId === data) { assignedPosition = champ.position }
  })
  if (assignedPosition === '' || assignedPosition === undefined) { assignedPosition = 'ARAM' }
  let rune = await authenticate.createHttp1Request({
    method: 'GET',
    url: '/lol-perks/v1/currentpage'
  }, lcu.credentials)
  const databaseRune = await dataBase.client.from('runes').select('*').eq('champion_id', data).eq('lane', assignedPosition).order('count', 'desc')
  const stats = []
  for (let i = 0; i < databaseRune.data.length; i++) {
    if (databaseRune.data[i].count < 10) { stats.push(0) } else { stats.push(databaseRune.data[i].count + databaseRune.data[i].winrate) }
  }
  const max = Math.max(...stats)
  const index = stats.indexOf(max)
  if (databaseRune.data.length === 0) { rune.name = 'Not Found' } else {
    const text = rune._raw.toString()
    rune = JSON.parse(text)
    rune = await parseDataBaseRune(databaseRune, data, index, rune)
  }
  const newrune = new Rune(rune.primaryStyleId, rune.subStyleId, rune.selectedPerkIds, rune.name)
  sendRuneToLc(newrune)
...
async function sendRunes (gameId) {
  ...
  this.gameId = gameId
  const gamess = await dataBase.client.from('games').select('*').eq('gameid', gameId)
