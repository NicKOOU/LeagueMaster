...
class Rune {
  constructor (championId, lane, primarystyleid, primary1, primary2, primary3, primary4, substyleid, sub1, sub2, win) {
  ...
  async send () {
    if (this.primary1 === 0) { return }
    const count = await db.client.from('runes').select('*').eq('champion_id', this.championId).eq('lane', this.lane).eq('primarystyleid', this.primarystyleid).eq('primary1', this.primary1).eq('primary2', this.primary2).eq('primary3', this.primary3).eq('primary4', this.primary4).eq('substyleid', this.substyleid).eq('sub1', this.sub1).eq('sub2', this.sub2)
      .then(pgres => pgres.data.length)
      .catch(err => console.log(err))
    if (count > 0) {
      const number = await db.client.from('runes').select('*').eq('champion_id', this.championId).eq('lane', this.lane).eq('primarystyleid', this.primarystyleid).eq('primary1', this.primary1).eq('primary2', this.primary2).eq('primary3', this.primary3).eq('primary4', this.primary4).eq('substyleid', this.substyleid).eq('sub1', this.sub1).eq('sub2', this.sub2)
        .then(pgres => pgres.data[0].count)
        .catch(err => console.log(err))

      await db.client.from('runes').update({ count: number + 1 }).eq('champion_id', this.championId).eq('lane', this.lane).eq('primarystyleid', this.primarystyleid).eq('primary1', this.primary1).eq('primary2', this.primary2).eq('primary3', this.primary3).eq('primary4', this.primary4).eq('substyleid', this.substyleid).eq('sub1', this.sub1).eq('sub2', this.sub2)
    } else { await db.client.from('runes').insert([{ champion_id: this.championId, lane: this.lane, primarystyleid: this.primarystyleid, primary1: this.primary1, primary2: this.primary2, primary3: this.primary3, primary4: this.primary4, substyleid: this.substyleid, sub1: this.sub1, sub2: this.sub2 }]) }
    if (this.win === true) {
      const wina = await db.client.from('runes').select('*').eq('champion_id', this.championId).eq('lane', this.lane).eq('primarystyleid', this.primarystyleid).eq('primary1', this.primary1).eq('primary2', this.primary2).eq('primary3', this.primary3).eq('primary4', this.primary4).eq('substyleid', this.substyleid).eq('sub1', this.sub1).eq('sub2', this.sub2)
        .then(pgres => pgres.data[0].win)
        .catch(err => console.log(err))
      await db.client.from('runes').update({ win: wina + 1 }).eq('champion_id', this.championId).eq('lane', this.lane).eq('primarystyleid', this.primarystyleid).eq('primary1', this.primary1).eq('primary2', this.primary2).eq('primary3', this.primary3).eq('primary4', this.primary4).eq('substyleid', this.substyleid).eq('sub1', this.sub1).eq('sub2', this.sub2)
    }
    const gameCount = await db.client.from('runes').select('*').eq('champion_id', this.championId).eq('lane', this.lane).eq('primarystyleid', this.primarystyleid).eq('primary1', this.primary1).eq('primary2', this.primary2).eq('primary3', this.primary3).eq('primary4', this.primary4).eq('substyleid', this.substyleid).eq('sub1', this.sub1).eq('sub2', this.sub2)
      .then(pgres => pgres.data[0].count)
      .catch(err => console.log(err))
    const win = await db.client.from('runes').select('*').eq('champion_id', this.championId).eq('lane', this.lane).eq('primarystyleid', this.primarystyleid).eq('primary1', this.primary1).eq('primary2', this.primary2).eq('primary3', this.primary3).eq('primary4', this.primary4).eq('substyleid', this.substyleid).eq('sub1', this.sub1).eq('sub2', this.sub2)
      .then(pgres => pgres.data[0].win)
      .catch(err => console.log(err))
    const winrate = win / gameCount * 100
    console.log('winrate: ' + winrate)
    await db.client.from('runes').update({ winrate }).eq('champion_id', this.championId).eq('lane', this.lane).eq('primarystyleid', this.primarystyleid).eq('primary1', this.primary1).eq('primary2', this.primary2).eq('primary3', this.primary3).eq('primary4', this.primary4).eq('substyleid', this.substyleid).eq('sub1', this.sub1).eq('sub2', this.sub2)
  }
}

function assignlane (ip) {
  if (ip === 'Invalid') { return 'ARAM' }
  switch (ip) {
    case 'TOP':
      return 'top'
    case 'JUNGLE':
